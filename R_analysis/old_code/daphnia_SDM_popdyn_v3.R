...
for (i in 1:nspp ) {
  daph_spp[[i]] = subset(spp_gbif[[i]][ , c("country" , "acceptedScientificName" , "basisOfRecord",  "datasetName", "genus", "geodeticDatum", "lat", "lon", "month", "occurrenceStatus" , "scientificName" , "species" , "year")]) 
}
...
for (i in 1:nspp ) {
  ...
  daph_spp[[i]] = subset(daph_spp[[i]], !is.na(lon) & !is.na(lat) & !is.na(year))
...
bp_delta =data.frame( c(raster::as.matrix(daphC_delta_pop[[1]])), c(raster::as.matrix(daphC_delta_pop[[2]])),c(raster::as.matrix(daphC_delta_pop[[3]])), c(raster::as.matrix(daphC_delta_pop[[4]])) )
...
suit_quant = vector(mode = "list",nspp)
for (i in 1:nspp){
  suit_quant[[i]] = quantile(daphC_pred_future[[i]])
}
...
for (i in 1:nspp){
  marg_points[[i]] = daphC_pred_future[[i]]<= suit_marg[[i]][1]
  inter_points[[i]] = (daphC_pred_future[[i]] > suit_inter[[i]][1] & 
                         daphC_pred_future[[i]] <= suit_inter[[i]][2])
  core_points[[i]] = daphC_pred_future[[i]] > suit_core[[i]][1]
}
...
bp_delta =data.frame( c(raster::as.matrix(daphC_delta_pop_fut[[1]])), c(raster::as.matrix(daphC_delta_pop_fut[[2]])),c(raster::as.matrix(daphC_delta_pop_fut[[3]])),c(raster::as.matrix(daphC_delta_pop_fut[[3]])) )
colnames(bp_delta) = c(1:nspp)

fig.name = paste("daphnia_delta_pop_fut1.jpg",sep="")
jpeg(file=fig.name, family='Arial', pointsize=16)
...
###Then this is the part of the range where most of that change happens: 
fig.name = paste("daphnia_delta_rangeparts_fut1.jpg",sep="")
jpeg(file=fig.name, family='Arial', pointsize=16)

par(mfrow = c(1,nspp))
for(i in 1 : nspp){
  dp_tmp = c(raster::as.matrix(daphC_delta_pop_fut[[i]]))
  bp_tmp = list(m = dp_tmp[c(raster::as.matrix(marg_points[[i]]) )] , 
                i = dp_tmp[c(raster::as.matrix(inter_points[[i]]) )],
                c = dp_tmp[c(raster::as.matrix(core_points[[i]]) )])
  if( i == 1) { 
